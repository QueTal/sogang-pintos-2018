...
//  lock_init(&load_lock);
 ...
 // sema_down(&(thread_current()->load)); 
  ...
  for(e = list_begin(&thread_current()->child); e!=list_end(&thread_current()->child); e = list_next(e)){
	...
	if(child_thread->load_flag==true){
...
/* Waits for thread TID to die and returns its exit status.  If
   ...
   does nothing. */
struct thread *get_child_process(int tid, struct thread *t){
	struct thread *child_thread;
	struct list_elem *e;

	for(e = list_begin(&t->child); e!=list_end(&t->child); e = list_next(e)){
		child_thread = list_entry(e, struct thread, child_elem);
		if(child_thread->tid==tid)return child_thread;
	}
	return NULL;
}
int
process_wait (tid_t child_tid) 
{
	...
	struct thread *child;
	int temp_exit_status = -1;


	child = get_child_process(child_tid, cur);
	if(child!=NULL){
		sema_down(&child->wait);
		temp_exit_status = child->exit_status;
...
void make_stack(char** argv, int argc, void** esp){
	...
	if( (first - (uint32_t)(*esp)) % WORDSTEP != 0){
		*esp -= (first - (uint32_t)(*esp) ) % 4;
	}
	
	*esp -= WORDSTEP; memset(*esp, 0, WORDSTEP);
	...
	*esp -=WORDSTEP; **(uint32_t **)esp = (uint32_t)*esp + WORDSTEP;
	*esp -=WORDSTEP; **(uint32_t **)esp = argc;
	*esp -=WORDSTEP; **(uint32_t **)esp = 0;
}
...
int tokenize(char *argv[MAXARGNUM], char *filename){
	char *saveptr;
	int argc = 0;
	int i;
	argv[argc] = strtok_r(filename, " ", &saveptr);
	for( i = 0; ; i++){
